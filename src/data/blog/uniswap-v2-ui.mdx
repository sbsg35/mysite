...
If you are planning on forking Uniswap V2 and you want custom frontend detached from the
[Uniswap V2 interface codebase](https://github.com/Uniswap/interface/releases/tag/v2.6.5), this post is for you.
I will be doing a post which goes deep into the code and Uniswap concepts.
For now, this is a high level approach and I assume you know the basics of Uniswap.
...
I originally forked the [Uniswap V2 interface](https://github.com/Uniswap/interface/releases/tag/v2.6.5)
repository and it worked fairly seamlessly with our contracts after some wrangling. Soon however there were some problems:

- Codebase was quite outdated
...
- Didn't fit in with our existing tech stack
- It was very slow
...
Obviously there are a gazillion forks of Uniswap interface, but we wanted to build from scratch. The [article
by Ben Haslam](https://medium.com/clearmatics/how-i-made-a-uniswap-interface-from-scratch-b51e1027ca87) was a really good starting point.
My main concern with this approach was that it was using direct web3 calls to fetch the swap prices with the `getAmountsOut` method.
In the SDK there are helper functions which calculate this price information based on the reserves in the pairs involved in the trade.
So we don't need to do constant debounced web3 calls as long as our pair information is up to date.
...
Below are the high level methods you need from the Uniswap SDK to execute a swap.
...
This will find the best path for a trade. It returns a `Trade` object,
